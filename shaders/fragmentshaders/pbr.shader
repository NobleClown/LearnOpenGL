...
float G_S_GGX(float NdotV, float roughness) {
...
float G_Smith(vec3 N, vec3 V, vec3 L, float roughness) {
    ...
    float ggx1 = G_S_GGX(NdotV, roughness);   // 观察方向上的几何遮蔽(看不到)
    float ggx2 = G_S_GGX(NdotL, roughness);   // 光线方向上的几何阴影(阴影中)
...
vec3 Fresnel(vec3 N, vec3 V, vec3 albedo, float metalness, float roughness) {
    vec3 F0 = vec3(0.04);   // 电介质近似基础反射率
    F0 = mix(F0, albedo, metalness);
    float cosTheta = dot(N, V);
    return F_Schlick(cosTheta, F0, roughness);
}
...
void main() {
    ...
    for (int i=0; i<4; i++) {
        vec3 lightPos = vec3(view * vec4(pointLights[i].position, 1.0));
        ...
        // 计算近似的反射率和折射率
        vec3 F = Fresnel(N, V, albedo, metallic, roughness);
        // 根据微表面法线分布计算法线与半程向量的接近程度
        float NDF = D_GGX_TR(N, H, roughness);
        // 计算微表面互相遮挡的程度
        float G = G_Smith(N, V, L, roughness);
        vec3 nominator = NDF * G * F;
        float denominator = 4.0 * max(dot(N, V), 0.0) * max(dot(L, N), 0.0) + 0.001;
    ...
    // 环境光
    vec3 kS = Fresnel(N, V, albedo, metallic, roughness);
    vec3 F = kS;
